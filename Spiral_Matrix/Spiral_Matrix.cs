   ...
   class Spiral_Matrix
   {
      ...
      static void Print(int row, int col, char[,] matrix)
      {
         if (col * 2 + 4 >= Console.WindowWidth)
         {
            ...
            Console.Write("--- Spiral matrix ---");
            Console.SetCursorPosition(6, 1);
            Console.Write("Matrix heigth: {0}", matrixHeigth);
            Console.SetCursorPosition(6, 2);
            Console.Write("Matrix height: {0}", matrixLength);
            Console.SetCursorPosition(6, 3);
            Console.Write("Matrix speed: {0}", sleepTime);
            return;
         }
         matrix[row, col] = matrixSymbol;
         Console.SetCursorPosition(col * 2 + 4, row + 5);
      ...
      static void Main(string[] args)
      {
         ...
         Console.WindowHeight = matrixHeigth + 6;
